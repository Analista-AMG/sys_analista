...
def CargaNominaMesaSoporte(fecha_fin):
    ...
    with engine.connect() as connection:
        with connection.begin() as transaction:
            ...
            transaction.commit()
    archivo_excel = fr"\\SRV-FS\Repositorio_Camp_AMG\Mesa_de_Soporte\{año}\{mesNombre}\nomina_{hoy.strftime('%Y%m')}.xlsx"
    ...
    if df_final is not None:
        try:
            ...
            excel_file_path = fr"\\SRV-FS\Repositorio_Camp_AMG\Mesa_de_Soporte\{año}\{mesNombre}\Consolidado {mesNombre}.xlsx"
    ...
    try:
        ...
        cursor.callproc('Sp_ActualizacionNomina_MesaSoporte', parametros)
...
def Campaña_MesaSoporte_Navicat(fecha_inicio, fecha_fin):
    ...
    try:
        ...
        with engine.connect() as connection:
            with connection.begin():
                connection.execute(
                    text("""
                        DELETE FROM TblMesasoporteExcel
                    """)
                )
        ...
        # Conexión a Navicat
        conexion = conectar_bdStores_Navicat2()
        cursor = conexion.cursor()
        ...
        query = f"""
                ...
                WHERE convert(`FECHA REGISTRO`,date) >= '{fecha_ini.strftime('%Y-%m-%d')}'
                AND convert(`FECHA REGISTRO`,date) <= '{fecha_f.strftime('%Y-%m-%d')}'
        """
        ...
        df.columns = [normalize_column_name(col) for col in df.columns]
        df['dni_tecnico'] = df['dni_tecnico'].astype(str).str.replace(r'[a-zA-Z]', '0', regex=True)
    ...
    try:
        ...
        cursor.callproc('sp_navicat_MesaSoporte', parametros)
        conexion.commit()
        logging.info("Procedimiento almacenado sp_navicat_MesaSoporte ejecutado correctamente.")
    except Exception as e:
...
def Interacciones_MesaSoporte(fecha_inicio, fecha_fin):
    try:
        locale.setlocale(locale.LC_TIME, 'es_ES.UTF-8')  # Linux
    except locale.Error:
        try:
    ...
    with engine.connect() as connection:
        ...
        with connection.begin() as transaction:
            connection.execute(text(f"DELETE FROM TblInteraccionesExcel where cola in ('VQ_OUTAMGCLOSETHELOOP1','VQ_OUTAMGCLOSETHELOOP2')"  ))
            transaction.commit()
    ...
    for dia in range(dia_inicio, dia_fin + 1):
        ruta_InteraccionesMesa_Soporte = fr"\\SRV-FS\AnalistaControlGestion\DESCARGA INFORMES\{año}\Mesa Soporte HFC\Interacciones\{mesNombre}\interacciones{dia:02d}.csv"


        if os.path.exists(ruta_InteraccionesMesa_Soporte):
            try:
                
                df_merged_Interacciones_Multi = pd.read_csv(ruta_InteraccionesMesa_Soporte,sep=',')
                ...
                logging.info(f"Archivo Interacciones cargado exitosamente para el día {dia}: {ruta_InteraccionesMesa_Soporte}")
            except Exception as e:
                logging.error(f"Error procesando archivo Interacciones {ruta_InteraccionesMesa_Soporte} para el día {dia}: {e}")
        else:
            logging.warning(f"Archivo Interacciones no encontrado para el día {dia}: {ruta_InteraccionesMesa_Soporte}")
    ...
    try:
        cursor = conexion.cursor()
        parametros = (fecha_ini, fecha_f)
        cursor.callproc('sp_InteraccionesMesaSoporte', parametros)
        conexion.commit()
...
def KpiMesaSoporte(fecha_fin):
    ...
    try:
        cursor = conexion.cursor()  
        parametros = (año,mes )
        cursor.callproc('SP_KpiMesaSoporte', parametros)
        conexion.commit()
        logging.info("Procedimiento almacenado Sp_CampañaDelivery_Rango ejecutado correctamente.")
    except Exception as e:
        import traceback
